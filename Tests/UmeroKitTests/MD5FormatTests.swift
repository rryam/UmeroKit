...
struct MD5FormatTests {
  ...
  func formatWithLeadingZeros() {
    ...
    let startIndex = hash.index(hash.startIndex, offsetBy: 8)
    let endIndex = hash.index(startIndex, offsetBy: 2)
    let substring = String(hash[startIndex..<endIndex])
    #expect(substring == "a0")
  }
  ...
  func lastFmExampleSignature() {
    ...
    let hash2 = Insecure.MD5.hash(data: data)
